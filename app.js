...
【明日计划】场景：
- 禁止使用"完成"、"已完成"、"产出"、"交付"等结果性词汇，这是计划而非已完成的工作
- 必须使用"计划进行"、"计划推进"、"计划开展"、"着手进行"等计划性表述
- 句末状态标识使用"预计完成"而非"已完成"
- 若用户输入包含百分比，句末添加（预计完成XX%），使用用户输入的百分比
- 若用户输入无百分比，句末添加（预计完成100%）
- 不要在句子中间描述进度，百分比只出现在句末括号内
- 示例输入：云农谷，AI产量预测，原型设计，30%
- 示例输出：根据平台运营发展规划，计划进行云农谷平台「AI产量预测」功能的原型设计工作。（预计完成30%）
- 示例输入：实现用户中心
- 示例输出：计划推进「用户中心」功能的开发与实现工作。（预计完成100%）`;
